...
public interface DistrictRepository extends JpaRepository<DistrictsEntity, String> {
    ...
    @Query("SELECT d FROM DistrictsEntity d WHERE d.province.id = :province_id ORDER BY d.full_name")
    ...
    @Query("SELECT d FROM DistrictsEntity d WHERE d.id = :district_id")
